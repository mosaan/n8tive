...
- セキュリティとアクセス制御: デフォルトはローカルアクセスのみを許可し、設定によって外部アクセスを明示的に許可できるようにする（将来的にアクセス元 IP 制限やバインドアドレス変更を UI/設定で切替）。
...
6. **ポート変更とセルフリカバリ**: ポート競合時に使用ポートを UI に表示し自動リトライ、Electron メニューからポート番号を指定して再起動できる設定を提供。設定したポート番号は `app.getPath('userData')/n8tive_config.json` に保存して次回起動に反映。ユーザー未指定時のみデフォルトポートから +1 ずつ探索し、明示指定時は競合があればエラー表示で再設定を促す。メニュー/ダイアログで「自動設定に戻す」を選ぶと保存済みポートを消去し、自動探索に復帰。
7. **ログフォルダビューア**: システムトレイメニューおよびアプリケーションメニューバー（Help > View Logs...）から、n8nログディレクトリ（`app.getPath('userData')/logs/`）をOSの標準ファイルエクスプローラーで開く。ログディレクトリが存在しない場合はエラーダイアログを表示。
